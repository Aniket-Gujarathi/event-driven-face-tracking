/**
 * @Author: aniket
 * @Date:   2019-07-18T15:55:55+05:30
 * @Last modified by:   aniket
 * @Last modified time: 2019-08-07T13:19:20+05:30
 */
                                 ...
                                 bool adaptive, int nthreads)
{
    ...
    vector<double> mins = {0.0, 0.0, 0.9};
    vector<double> maxs = {(double)res.width, (double)res.height, 1.2};
    p.setConstraints(mins, maxs);

    //double max_likelihood = initialiseAsCircle(30);
    double max_likelihood = faceTemplate(80, 99);
...
double vParticlefilter::faceTemplate(int ox, int oy) //*
{
    std::array<std::array<double, 160>, 199> data;
    std::ifstream inFile; //*

    inFile.open("/home/aniket/Desktop/Event-Driven-Tracking/Matlab/face.txt");
    ...
    //appearance = new ImageOf<PixelFloat> [199 * 160 * sizeof(ImageOf<PixelFloat>)];
    appearance.resize(ox*2, oy*2 + 1);
    appearance.zero();
    offset_x = ox;
    offset_y = oy;
    double neg_region = -1;
